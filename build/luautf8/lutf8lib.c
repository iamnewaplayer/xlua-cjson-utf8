...
static int utf8_range(const char *s, const char *e, lua_Integer *i, lua_Integer *j) {
  /* Support for xlua 64-bit integers with proper range checking */
    int i_val = (*i > INT32_MAX || *i < INT32_MIN) ? INT_MAX : CAST(int, *i);
    int j_val = (*j > INT32_MAX || *j < INT32_MIN) ? INT_MAX : CAST(int, *j);
    const char *ps = utf8_relat(s, e, i_val);
    const char *pe = utf8_relat(s, e, j_val);
  *i = (ps ? ps : (*i > 0 ? e : s)) - s;
...
static int Lutf8_byte (lua_State *L) {
  ...
  /* Support for xlua 64-bit integers with proper range checking */
    if (n > INT32_MAX) return INT_MAX;
    if (n < INT32_MIN) return INT_MIN;
    return CAST(int, n);
}
...
static int Lutf8_insert (lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TNUMBER) {
    /* Support for xlua 64-bit integers with proper range checking */
    lua_Integer lua_idx = lua_tointeger(L, 2);
    if (lua_idx > INT32_MAX || lua_idx < INT32_MIN) {
        lua_pushint64(L, (int64_t)lua_idx);
        return 1;
    }
    int idx = (int)lua_idx;
...
static int iter_aux (lua_State *L, int strict) {
  const char *e, *s = check_utf8(L, 1, &e);
  /* Support for xlua 64-bit integers with proper range checking */
    lua_Integer lua_n = lua_tointeger(L, 2);
    if (lua_n > INT32_MAX || lua_n < 0) {  /* n should be non-negative */
        return luaL_error(L, "invalid argument #2 to 'utf8.char' (non-negative number expected)");
    }
    int n = CAST(int, lua_n);
...
static int width_opt (lua_State *L, int idx, int *pdefault) {
  /* Support for xlua 64-bit integers with proper range checking */
    lua_Integer ambiwidth_int = luaL_optinteger(L, idx, 1);
    int ambiwidth = (ambiwidth_int > INT32_MAX || ambiwidth_int < 0) ? 1 : CAST(int, ambiwidth_int);
    if (pdefault != NULL) {
        lua_Integer default_int = luaL_optinteger(L, idx+1, 0);
        *pdefault = (default_int > INT32_MAX || default_int < 0) ? 0 : CAST(int, default_int);
    }
  return ambiwidth;
}
...
static int Lutf8_widthindex (lua_State *L) {
  ...
  const char *e, *s = luaL_checklstring(L, 1, &len);
  /* Support for xlua 64-bit integers with proper range checking */
    lua_Integer width_int = luaL_checkinteger(L, 2);
    if (width_int > INT32_MAX || width_int < 0) {
        return luaL_error(L, "invalid argument #2 to 'utf8.width' (non-negative number expected)");
    }
    int chwidth, width = CAST(int, width_int);
...
static int find_aux (lua_State *L, int find) {
  ...
  if (!idx) idx = 1;
  /* Support for xlua 64-bit integers with proper range checking */
    int idx_val = (idx > INT32_MAX || idx < INT32_MIN) ? 0 : CAST(int, idx);
    init = utf8_relat(s, es, idx_val);
  if (init == NULL) {
    if (idx > 0) {
